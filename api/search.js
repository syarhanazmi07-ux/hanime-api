...
export default async function handler(req, res) {
  ...
  try {
    ...
    const r = await fetch(`${EPORNER}/video/search/?query=${encodeURIComponent(q)}&per_page=30&page=${page}&format=json&thumbsize=big`);
    const data = await r.json();
    res.json({
      results: (data.videos || []).map(fmt),
      query: q, page, total: data.total_count,
